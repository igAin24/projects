...
const elmenentAdd = document.getElementById("add");
const elmenentSub = document.getElementById("sub");
const elmenentMult = document.getElementById("mult");
const elmenentDiv = document.getElementById("div");
const elmenentEqual = document.getElementById("equal");
const elementCancle = document.getElementById("cancel");
// イベントを登録
// 【Ｂ】自分で考える
...
elmenentAdd.addEventListener("click",function(){update("+")});
elmenentSub.addEventListener("click",function(){update("-")});
elmenentMult.addEventListener("click",function(){update("*")});
elmenentDiv.addEventListener("click",function(){update("/")});
elmenentEqual.addEventListener("click",dspResult);
elementCancle.addEventListener("click",clear);
/** 数字がクリックされたときの処理 */
...
function update(calcType) {
  ...
  // １つ前の入力が演算子（演算子 入力しなおし）
    ...
    // 初回入力
    if (wkFirst === "1") {
      elementcalcLog.innerHTML = "0" + elementcalcType; //計算ログ
    }
    else {
      // 演算子 入力しなおし
